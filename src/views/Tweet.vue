<template>
  <v-container class="ma-0 pa-0">
    <!-- top nav -->
    <v-card tile flat class="pa-3 align-center" style="border-bottom: thin solid rgba(0, 0, 0, 0.12)">
    ...
    <v-card tile flat class="mx-3 py-3 align-center grey--text" style="
        font-size: 16px;
        border-bottom: thin solid rgba(0, 0, 0, 0.12);
        border-top: thin solid rgba(0, 0, 0, 0.12);
      ">
      <v-row class="pa-0 ma-0">
        <v-col cols="2" class="pa-0 ma-0"><span class="black--text">{{ tweet.replyCounts }}</span> 回覆</v-col>
        <v-col cols="10" class="pa-0 ma-0"><span class="black--text">{{ tweet.likeCounts }}</span>
          喜歡次數</v-col>
      </v-row>
    </v-card>
    <!-- replies and likes icons -->
    <v-card tile flat class="pa-3 align-center grey--text" style="border-bottom: thin solid rgba(0, 0, 0, 0.12)">
      <v-row class="pa-0 ma-0">
    ...
    <!-- replies -->
    <v-card v-for="reply in replies" :key="reply.id" flat tile class="pa-3" style="border-bottom: thin solid rgba(0, 0, 0, 0.12)">
      <v-row no-gutters class="pa-0" style="flex-wrap: nowrap">
        ...
        <v-col cols="11" class="flex-shrink-1">
          <v-row no-gutters align="center" class="pa-2">
            ...
            <router-link :to="{ name: 'user', params: { id: reply.User.id } }" class="subtitle-2 grey--text font-weight-normal pa-0" style="text-decoration: none">
              @{{ reply.User.account }}
            </router-link>
            <p class="subtitle-2 font-weight-normal grey--text pa-0 my-0 ml-1">
          ...
          <v-row no-gutters align="center" class="font-weight-normal px-2 mb-2">
            <p class="body-2 font-weight-normal text-justify grey--text my-0">
              回覆
              <router-link :to="{ name: 'user', params: { id: tweet.User.id } }" style="text-decoration: none">@{{ tweet.User.name }}</router-link>
            </p>
          </v-row>
